...
class ContactFormHandler {
  ...
  async handleSubmit(event) {
    ...
    try {
      if (this.config.useAjax && this.config.submitUrl) {
        await this.ajaxSubmit();
      } else {
      ...
      if (this.config.onSuccess) {
        this.config.onSuccess(this.getFormData());
      }
    } catch (error) {
...
class DemandFilter {
  ...
  init() {
    ...
    if (!this.validateElements()) {
      console.error("DemandFilter: Required elements not found");
      return;
    }
  ...
  validateElements() {
    if (!this.container) {
      console.error(
        `DemandFilter: Container with id "${this.config.containerId}" not found`,
      );
      return false;
    }
    ...
    if (this.filterButtons.length === 0) {
      console.error(
        `DemandFilter: No filter buttons found with selector "${this.config.filterButtonsSelector}"`,
      );
      return false;
    }
...
class BackToTop {
  ...
  init() {
    ...
    if (!this.$backToTop.length) return; // safety
...
// Initialise both — FAQAccordion is untouched
